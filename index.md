...
&Algoritmo Filtro de Bayes (bel(x_{t-1}), u_t, z_t):& \\
    &for \hspace{0.2cm} all \hspace{0.2cm} x_t \hspace{0.2cm}faça \\
        &\hspace{0.8cm}\overline{bel}(x_t) = \int p(x_t | u_t, x_{t-1}) \, \text{bel}(x_{t-1}) \, dx_{t-1}& \\
        &\hspace{0.8cm}bel(x_t) = \eta \, p(z_t | x_t) \, \text{bel}(x_t)& \\ 
   &fimfor \\
 &return bel(x_t)&
\end{aligned}
$$
